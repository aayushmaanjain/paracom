...
int main(int argc, char *argv[])
{
	...
	// sendsizes = (unsigned long int *)malloc(1*sizeof(unsigned long int));
	// sendsizes[rank] = 1;
	// recvsizes = (unsigned long int *)malloc(1*sizeof(unsigned long int));
	// recvsizes[rank] = 1;
	...
	// }
	for(i=1;i<numproc;i++)	{	// i = cyclic interval
		dest = (rank + i) % numproc;
		src = (rank - i + numproc) % numproc;
		// non blocking receiving the size of char array from src
		recvtag = src*100 + rank;
		MPI_Irecv(&recvsizes,1,MPI_UNSIGNED_LONG,src, recvtag,MPI_COMM_WORLD,&recvrequest);
		// create char array to send
		// serialize
		cout<<i<<"\t"<<rank<<"\tdest: "<<dest<<"\tsrc: "<<src<<"\tbreak1"<<endl<<flush;
		// get size of char array to send
		sendsizes = 100 + rank + dest;	// DO : change
		// send buffer
		sendbuf = (char *)malloc(sendsizes*sizeof(char));
		string s("HELLO");
		if((rank==0) && (dest==1))
		strcpy(sendbuf,s.c_str());	// DO : directly put serialization result in sendbuf
		// sending the size of char array
		sendtag = rank * 100 + dest;
		MPI_Isend(&sendsizes,1,MPI_UNSIGNED_LONG,dest,sendtag,MPI_COMM_WORLD,&sendrequest);
		cout<<i<<"\t"<<rank<<"\tdest: "<<dest<<"\tsrc: "<<src<<"\tbreak2"<<endl<<flush;
		
		
		// wait for receive to complete
		MPI_Wait(&recvrequest, MPI_STATUS_IGNORE);
		// allocate memory for recvbuf
		recvbuf = (char *)malloc(recvsizes*sizeof(char));
		// receive serialized stream
		recvtag += 100000;
		// blocking receive the serialized stream
		MPI_Irecv(&recvbuf[0], recvsizes, MPI_CHAR, src, recvtag, MPI_COMM_WORLD, &recvrequest1);
		// send the char array
		sendtag += 100000;
		MPI_Isend(&sendbuf[0], sendsizes, MPI_CHAR, dest, sendtag, MPI_COMM_WORLD, &sendrequest1);
		MPI_Wait(&recvrequest1,MPI_STATUS_IGNORE);
		// deserialize

		// merge into globalindex
		if((rank==1) && (src==0))
			cout<<"Received: "<<recvbuf<<endl;
		//free up receive buffer after processing
		free(recvbuf);
		// wait on send to complete
		MPI_Wait(&sendrequest, MPI_STATUS_IGNORE);
		MPI_Wait(&sendrequest1, MPI_STATUS_IGNORE);
		free(sendbuf);
	}
